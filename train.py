...
if __name__ == '__main__':
    ...
    model = keras.models.load_model('hairnet_matting3.hdf5') #Hairnet.get_model()
    model.compile(optimizer=Adam(lr=1e-4), loss='binary_crossentropy', metrics=['accuracy'])

    model_checkpoint = ModelCheckpoint('hairnet_matting.hdf5', monitor='loss', verbose=1, save_best_only=True)
    model.fit_generator(myGene, callbacks=[model_checkpoint], steps_per_epoch=2000, epochs=10)